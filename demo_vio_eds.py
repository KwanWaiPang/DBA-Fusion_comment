...
def image_stream(imagedir, imagestamp, enable_h5, h5path, calib, stride):
    ...
    if not enable_h5:#如果不是h5文件
        ...
        for t, imfile in enumerate(image_list):
            ...
            tt = image_stamps[t] /1e6 #时间戳,转换为秒，读入的时间戳是纳秒，t是索引
...
if __name__ == '__main__':
    ...
    print(f'\033[0;31;42m testing EDS!!! \033[0m')
    ...
    try:
        ...
        while True:
            ...
            # 第一个元素是时间戳
            sod = float(elem[0]) #时间戳（转换为秒，读入的时候是妙）
    ...
    for (t, image, intrinsics) in tqdm(image_stream(args.imagedir, args.imagestamp, args.enable_h5,\
        ...
        if dbaf is None:
            ...
            dbaf.frontend.all_stamp = dbaf.frontend.all_stamp[None].transpose(1,0)/1e6 #提取第一列转换为秒（微妙）
            if len(all_gt) > 0:
                # 设置真值数据
    ...
    if args.evaluate_flag:
        ...
        result = main_ape.ape(traj_ref, traj_est, 
                pose_relation=PoseRelation.translation_part, align=True,n_to_align=-1, correct_scale=True)#注意n_to_align=1000
        ...
        ate_score=ate_score*100

        res_str = f"\nATE[cm]: {ate_score:.03f} | MPE[%/m]: {MPE:.02f}"
